...
def api_get_presenze_studente(matricola):
    """
    Recupera le presenze di uno studente. Se l'ingresso avviene entro 20 minuti 
    dall'inizio della lezione, viene conteggiata l'intera durata della lezione.
    Altrimenti, viene calcolata dall'orario di ingresso effettivo.
    """
    conn = get_db_connection()
    presenze = []
    try:
            ...
            sql = """
                ...
                SELECT 
                    ...
                    COALESCE((
                        SELECT SUM(
                            TIME_TO_SEC(
                                -- INIZIO DELLA NUOVA LOGICA CONDIZIONALE
                                CASE
                                    -- Se l'orario di ingresso è DOPO l'inizio lezione + 20 minuti
                                    WHEN p.orario_ingresso > TIMESTAMPADD(MINUTE, 20, l.ora_inizio) 
                                    -- Allora calcola dall'ingresso effettivo
                                    THEN TIMEDIFF(l.ora_fine, p.orario_ingresso) 
                                    -- Altrimenti (se è puntuale) calcola l'intera durata della lezione
                                    ELSE TIMEDIFF(l.ora_fine, l.ora_inizio) 
                                END
                                -- FINE DELLA NUOVA LOGICA CONDIZIONALE
                            )
                        ) / 3600
                        FROM Presenza p
